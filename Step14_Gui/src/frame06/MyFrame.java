...
public class MyFrame extends JFrame implements ActionListener{
   ...
   public void actionPerformed(ActionEvent e) {
      //메소드의 매개변수로 전달되는 ActionEvent 객체에 이벤트에 대한 정보가 들어 있
	  //이 객체를 활용하면 어떤 버튼이 눌러 졌는지 구별할수 있다.
	  //e.getSource();  눌러진 버튼의 참조값이 object type 으로 리턴된다
	   
	   
	   //생성자의 지역변수에있는 값을  메소드에서 사용하고싶다면 필드에다가 담아두면
	   //어디서든 사용할수있다 객체안에서 어떠한값을 담아놓는 값
	   
	   
	   //눌러진 버튼의 참조값 얻어오기
	   Object which = e.getSource();
	   if(which == sendBtn) {
		   //메소드 안에서 생성자에서 선언된 지역변수를 참조할수 있을까?
