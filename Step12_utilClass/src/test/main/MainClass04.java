...
public class MainClass04 {
	...
	public static void main(String[] args) {
		...
		List<Car> cars = new ArrayList<Car>();  //Car객체의 참조값을 담을려고 생성
		//2.Car객체(3개) 를 생성해서 List 객체에 저장해 보세요.
		cars.add(new Car("김구라"));
		cars.add(new Car("해골"));
		cars.add(new Car("원숭이"));
		...
		//4.확장for문을 이용해서 List 객체에 저장된 모든 Car 객체의 drive() 메소드를 순서대로 호출해보세요
		for(Car tmp:cars) {
			tmp.drive();
		}
